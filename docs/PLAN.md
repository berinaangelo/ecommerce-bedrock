...
4. Build modules 1–5 in order against the WC REST/Store API. Each module gets its own `page-{slug}.php` template bootstrapping the shared `ecommerceApp` Angular module with a page-specific controller — WooCommerce already auto-creates a dedicated page per module (Shop/Cart/Checkout), so WordPress's own page routing is reused instead of adding an Angular router.
   ...
   - **Module 4: Checkout** — shipping address + optional login/account-creation + payment, via the Store API's checkout flow (`/cart/update-customer` → `/cart/select-shipping-rate` → `/checkout`). Built here against a temporary payment stand-in; real Stripe is its own dedicated step (#5 below), not part of this pass.
     ...
     - [x] 4.1 `page-checkout.php` (WP's `page-{slug}.php` convention, WooCommerce's auto-created Checkout page) + `CheckoutController` — contact/shipping address fields matching the mockup's fieldset (mockup's single "Full name" split into first/last — the Store API's address schema requires them separately). Country is fixed to `'US'` (the store's own base country) rather than a form field — no source in the mockup and this storefront isn't scoped for multi-country shipping.
   ...
   - **Module 5: Order confirmation** — shows what was ordered and that it's paid, using only the response from Module 4's checkout call (per the module table — no separate Store API call). Built right after Module 4 above — the checkout response it renders is a real WooCommerce order, just paid via the COD stand-in until step #5 lands.
     ...
     - [ ] 5.5 Live verification: confirm WooCommerce's order-confirmation email actually fires on a completed (COD) order.
